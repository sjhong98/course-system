...
export const apiSyncResponseHandler = <T, E extends ErrorLike>(apiResponse: SerializableResponse<T, E>, option?: { key?: string }): T => {
  return responseErrorHandler(apiResponse, option)
}

const responseErrorHandler = <T, E extends ErrorLike>(apiResponse: SerializableResponse<T, E>, option?: { key?: string }) => {
  const { key } = option ?? {}
