    ...
    return (
        <>
            <Row gap={4} className='absolute top-2 right-0'>
                ...
                <CourseHeaderButton onClick={handleSelectChange} active={isSelectable}>수강 신청</CourseHeaderButton>
            ...
            {isFilterOpen && (
                <Row className='absolute right-0 w-full py-2 bg-[var(--background)] z-[100] border-b border-[var(--background-tertiary)]' style={{ top: `${PAGE_TITLE_HEIGHT}px` }}>
                    {sortList.map((item) => (
                        <CheckBox key={item.value} label={item.label} className='flex-1' checked={sort === item.value} onChange={() => handleSortChange(item.value as CourseListSort)} />
                    ))}
                </Row>
            )}
        </>
